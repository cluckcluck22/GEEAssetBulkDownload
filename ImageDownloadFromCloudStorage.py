	...
	for aNotDoneFile in listOfNotDoneFiles:
		...
		if aNotDoneFile.replace(localStorageLocationForFiles+"/","") in taskListNames:
			...
			if imgName in listOfRunningTasks:
				...
				print(aNotDoneFile.replace(localStorageLocationForFiles,"gs://bulcstorage"))
				existenceCheck = subprocess.Popen(["gsutil","du",aNotDoneFile.replace(localStorageLocationForFiles,"gs://bulcstorage")],stdout=subprocess.PIPE).stdout.read()
				if len(existenceCheck) > 0:
					print("File found on cloud storage, downloading now")
					outputLocation = aNotDoneFile.replace(imgName,".")
					print(outputLocation)
					subprocess.Popen(["gsutil","cp","-r",aNotDoneFile.replace(localStorageLocationForFiles,"gs://bulcstorage"),outputLocation],stdout=subprocess.PIPE).stdout.read()
					subprocess.Popen(["gsutil","rm","-r",aNotDoneFile.replace(localStorageLocationForFiles,"gs://bulcstorage")],stdout=subprocess.PIPE).stdout.read()
